...
public class ReflectBreakHandler {
    ...
    public boolean isBreakActive() {
        Object o = performReflection("isBreakActive0");
        if (o != null) {
            return (boolean) o;
        }
    ...
    private boolean checkReflection() {
        ...
        for (Plugin p : pluginManager.getPlugins()) {
            if (p.getClass().getSimpleName().equalsIgnoreCase("solacebreakhandlerplugin")) {
                for (Field f : p.getClass().getDeclaredFields()) {
                    if (f.getName().equalsIgnoreCase("solacebreakhandler")) {
